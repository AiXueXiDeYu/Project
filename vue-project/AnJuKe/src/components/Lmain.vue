    ...
    <div class="content">
        <div v-if="state.show" class="sign-in">
            <van-form @submit="onSubmit" ref="login">
                <van-cell-group inset>
                    <van-field v-model="state.username" name="username" label="用户名" placeholder="用户名" colon="true"
                        :rules="[{ required: true, message: '请填写用户名' }]" />
                    <van-field v-model="state.password" type="password" name="password" label="密码" clear placeholder="密码"
                        colon="true" :rules="[{ required: true, message: '请填写密码' }]" />
                </van-cell-group>
        ...
        <div class="sign-up" v-else>
            <van-form @submit="onSubmit" ref="register">
                <van-cell-group inset>
                    <van-field v-model="state.regname" name="regname" label="请输入用户名" label-width=100 placeholder="用户名"
                        colon="true" :rules="[{ required: true, message: '请填写用户名' }]" />
                    <van-field v-model="state.regpassword" type="password" name="regpassword" label-width=100 label="请输入密码"
                        placeholder="密码" colon="true" :rules="[{ required: true, message: '请填写密码' }]" />
                </van-cell-group>
                <div style="margin: 16px;">
                    <van-button round block type="primary" native-type="submit">
                        注册
